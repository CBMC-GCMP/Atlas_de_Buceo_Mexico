...
- "shp/MPA_MX_v1.01_21072021/MPA_MX_c1-01.shx"
- "shp/MPA_MX_v1.01_21072021/MPA_MX_c1-01.prj"
- "shp/MPA_MX_v1.01_21072021/MPA_MX_c1-01.dbf"
...
# Loading MPA layer -------------------
mpa <- raster::shapefile("shp/MPA_MX_v1.01_21072021/MPA_MX_c1-01.shp", encoding = "UTF-8", verbose = FALSE)
...
test <- st_join(dive_sites, mpa_v1.01)
...
leaflet(to) %>% 
  ...
  addPolygons(
        ...
        addLayersControl(
                baseGroups = c("Satélite", "Mapa", "Ocean"),
                overlayGroups = c("No especificado", "Sin manejo", "Prohibida", "Permitida"),
                options = layersControlOptions(collapsed = TRUE)
        )%>%
        hideGroup(c( "Prohibida", "No especificado", "Sin manejo")) %>%
        setView(lat =  21.601093,
                lng = -100.739661,
                zoom = 5)
...
bne <- subset(mpa, Buceo == "No se menciona")

bam <- subset(mpa, Buceo == "Ambiguo")
...
leaflet(to) %>%
  ...
  addLayersControl(
    baseGroups = c("Satélite", "Mapa", "Ocean"),
    overlayGroups = c("Ambiguo", "No se menciona", "Sin manejo", "Prohibido", "Permitido"),
    options = layersControlOptions(collapsed = TRUE)
  ) %>%
  hideGroup(c("Ambiguo", "Prohibida", "No se menciona", "Sin manejo")) %>%
  setView(lat =  21.601093,
          lng = -100.739661,
          zoom = 5)
...
renderPlot({
  mpa %>%
    ...
    mutate(Buceo = factor(
      Buceo,
      levels = c("No se menciona", "Sin manejo", "Prohibido", "Permitido", "Ambiguo")
    )) %>%
    ggplot(aes(
      x = reorder(Buceo, n),
      y = n,
    ...
    geom_col() +
    scale_fill_manual(values = c("#edb72f", "gray30", "#c91818", "#558019", "#c7c704"),
                      name = "Nivel de Protección") +
    theme_ipsum() +
    labs(x = "", y = "Número de subzonas en AMP") +
    theme(
...
mpa_v1.01 %>%
  filter(
    ...
    Buceo %in% c("Ambiguo", "No se menciona", "Sin manejo", "Permitido")
  ) %>%
  as.data.frame() %>%
...
subset(
Pesca %in% c("Permitida", "Sin manejo", "No especificado"),
Buceo %in% c("Ambiguo", "Sin manejo", "Permitido")
)
...
addLayersControl(
baseGroups = c("Satélite", "Mapa", "Ocean"),
overlayGroups = c("Solo Buceo", "Pesca y Buceo"),
options = layersControlOptions(collapsed = TRUE)
) %>%
hideGroup("Conflicto") %>%
setView(lat =  21.601093,
...
mpa %>% 
        as.data.frame() %>%
        filter(Pesca %in% c("Permitida", "Sin manejo", "No especificado"), Buceo %in% c("Ambiguo", "Sin manejo", "Permitido")) %>% 
        group_by(Pesca, Buceo) %>% 
        count() %>% 
        ungroup() %>% 
        summarise(n=sum(n))
...
mpa %>% 
        as.data.frame() %>% 
        filter(Pesca == "Prohibida", Buceo %in% c("Ambiguo", "Sin manejo", "Permitido")) %>% 
        group_by(Pesca, Buceo) %>% 
        count() %>% 
        ungroup() %>% 
        summarise(n=sum(n))
...
specie <- rep("AMP's",2)
category <- c("Pesca y Buceo","Solo Buceo")
value <- c(212,109)
actividad <- data.frame(specie, category, value)


renderPlot({
  ggplot(actividad, aes(fill=category, y=value, x=specie)) + 
...
<!--   mutate(Buceo = factor( -->
<!--     Buceo, -->
<!--     levels = c("Ambiguo", "No se menciona", "Sin manejo", "Prohibido", "Permitido") -->
<!--   )) %>% -->
<!--   mutate(Pesca = factor( -->
<!--     Pesca, -->
<!--     levels = c("No especificado", "Sin manejo", "Prohibida", "Permitida") -->
...
<!-- specie <- c(rep("Pesca", 5) , rep("Buceo" , 5)) -->
<!-- condition <- -->
<!--   rep(c("Ambiguo","No Especificado" , "Sin Manejo" , "Prohibido", "Permitido")) -->

<!-- data <- data.frame(specie, condition) -->

<!-- data <- data %>% -->
<!--   mutate(value = c(0,17, 41, 172, 222,66, 80, 41, 105, 226)) %>% -->
